...
export async function POST(request: NextRequest) {
  try {
      ...
      messages: [
        {
          role: 'system',
          content:
            "You are a helpful support assistant. Use the provided context from our Slack support channel to answer questions accurately and helpfully. If the context doesn't contain relevant information, provide general helpful guidance.",
        },
        {
          role: 'user',
          content: context,
    ...
    return NextResponse.json({
      ...
      slackContext:
        slackMessages.length > 0
          ? `Found ${slackMessages.length} relevant messages from support channel`
          : 'No relevant support messages found',
    });
  } catch (error) {
